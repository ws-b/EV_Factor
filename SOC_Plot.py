...
color_palette = [
    ...
    "#4dbbd5",  # 6
    "#ee4c97",  # 7
    "#7e6148",  # 8 (요청: Pack 관점 coulomb counting 표시)
    "#747678"   # 9
]
...
for csv_file in selected_files:
    ...
    # (2) cell_volt_avg / pack_volt_normalized
    ...
    # (B) 마지막에서 31~35번째 행까지 SOC를 확인하고, 0~100 범위 내 유효 값 찾기
    ...
    # - 시작 SOC(Cell, OCV)
    start_soc_cell_ocv = df["interp_soc"].iloc[0]
    # - SOC(Coulomb Counting, Cell)
    df["soc_cc_cell"] = (
        start_soc_cell_ocv
    ...
    # [Pack 관점] 계산
    # -------------------
    # - Pack 용량: 2병렬 이므로 (CELL_CAPACITY * 2), 그리고 SOH 적용
    PACK_CAPACITY = 2 * CELL_CAPACITY
    Q_pack = PACK_CAPACITY * df["soh"] / 100

    # - 전류: pack_current_corrected 그대로 사용
    ...
    # (1) cell_volt_avg + pack_volt_normalized
    axes[0].plot(df["elapsed"], df["cell_volt_avg"],
                 label="Cell Volt Avg", color=color_palette[0], alpha=0.7)
    ...
    #  - (b) SOC(Cell, OCV)의 처음/마지막 점
    axes[3].scatter(df["elapsed"].iloc[0],
                    df["interp_soc"].iloc[0],
                    color=color_palette[5],
                    marker='o', s=50,
                    label="SOC(Cell, OCV)")
    axes[3].scatter(df["elapsed"].iloc[-1],
                    df["interp_soc"].iloc[-1],
                    color=color_palette[5],
                    marker='o', s=50)

    #  - (c) SOC(Pack, OCV)의 처음/마지막 점
    axes[3].scatter(df["elapsed"].iloc[0],
    ...
    #  - (d) SOC(Coulomb Counting, Cell) 라인
    axes[3].plot(df["elapsed"], df["soc_cc_cell"],
                 label="SOC(CC, Cell)",  # 레이블 변경
                 color=color_palette[7], linestyle='--')

    #  - (e) SOC(Coulomb Counting, Pack) 라인 (추가, color_palette[8])
    ...
    print(f"  >> [Cell] Start SOC(Cell, OCV) = {start_soc_cell_ocv:.2f}%")
