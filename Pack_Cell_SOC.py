...
root_dir = r"D:\SamsungSTF\Processed_Data\TripByTrip_soc\Ioniq5"
...
# 4. 전역 points 리스트
# -------------------------------------------------------------------------------
points = []  # {"file": csv_file, "x": BMS_SOC, "y": Cell/Pack_SOC, "row_type": "first"/"last", "soc_type": "cell"/"pack"}
...
for csv_file in all_csv_files:
    ...
    # (C) cell_volt_avg / pack_volt_normalized
    ...
    # -------------------------------------------------------------------------------
    # (1) 'Start SOC'를 찾을 때:
    #     - 맨 앞쪽 30행(인덱스 0 ~ 29) 중에서
    #       cell_volt_avg가 3.0~4.4 안에 있는 첫 번째 행을 찾는다.
    # -------------------------------------------------------------------------------
    start_idx = None
    max_check_for_start = min(30, n_after_cut)  # 실제로 데이터가 30행보다 적을 수도 있음

    for i in range(max_check_for_start):
        avg_v = df["cell_volt_avg"].iloc[i]
        if (avg_v >= 3.0) and (avg_v <= 4.4):
            start_idx = i
            break
    ...
    # -------------------------------------------------------------------------------
    # (2) 'Final SOC'를 찾을 때:
    #     - 마지막에서 31~45번째 까지(= 총 15행) 중
    #       cell & pack SOC가 둘 다 [0..100] 범위 내인 행을 찾는다.
    #       (기존 31~35에서 31~45로 확장)
    # -------------------------------------------------------------------------------
    if n_after_cut < 45:
        # 마지막에서 45번째까지 보려면 최소 45행은 있어야 함
        print(f"[스킵] 뒤에서 45행도 안 됨(데이터 부족): {csv_file}")
        continue

    target_idx = n_after_cut - 31  # '기준'으로 삼을 인덱스 (원래 last - 31)
    found_valid = False
    # offset: 29..15 (step -1) => 총 15개
    #  예) offset=29 => source_idx= (n_after_cut-29) => "마지막에서 29번째"
    for offset in range(29, 14, -1):  # 29..15
        ...
        if 0 <= cell_val <= 100 and 0 <= pack_val <= 100:
            # target_idx 위치의 interp_soc/pack을 source_idx의 값으로 덮어씀
    ...
    # -------------------------------------------------------------------------------
    # 이제 elapsed 등 재설정(필요시)
    # -------------------------------------------------------------------------------
    df["elapsed"] = (df["time_dt"] - df["time_dt"].iloc[0]).dt.total_seconds()

    # -------------------------------------------------------------------------------
    # (4) 최종: 'Start SOC' (start_idx행)과 'Final SOC' (df.iloc[-1])을 사용
    # -------------------------------------------------------------------------------
    first_row = df.iloc[start_idx]
    last_row = df.iloc[-1]

    # local_points에 (Start,Cell), (Start,Pack), (Final,Cell), (Final,Pack)
    local_points.append({
        "file": csv_file,
